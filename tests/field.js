...
describe("数据验证", function () {
    ...
    it("验证数据的初始值（初始值给定错误）", function (done) {
        let foo = new F(2);
        foo.validate().then(
            (ret) => {
    ...
    it("验证数据的初始值（初始值给定正确）", function (done) {
        let foo = new F(8);
        foo.validate().then(
            (ret) => {
    ...
    it("赋值后验证数据(使用错误的初始值)", function (done) {
        let foo   = new F(3);
        foo.value = 6;
        foo.validate().then(
            (ret) => {
    ...
    it("赋值后验证数据(反复修改01)", function (done) {
        let foo   = new F(2);
        foo.value = 5;
        foo.value = 3;
        foo.validate().then(
    ...
    it("赋值后验证数据(反复修改02)", function (done) {
        let foo   = new F(2);
        foo.value = 5;
        foo.value = 7;
        foo.value = 8;
    ...
    it("赋值后验证数据(反复修改03)", function (done) {
        let foo   = new F();
        foo.value = 5;
        foo.value = 7;
        foo.value = 7;
...
describe("数组", function () {
        ...
        describe("修改数组后验证有效性", function (done) {
            let foo = new F([1, 2]);
            foo.value.push(3, 4);
            it("数据是否正确", function (done) {
                foo.validate().then((isValid) => {
